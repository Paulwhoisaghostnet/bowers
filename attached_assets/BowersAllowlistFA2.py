# BowersAllowlistFA2 - SmartPy v2
# Open Edition FA2 with allowlist phased minting + full marketplace.
# - Allowlist phase: only listed addresses can mint (per-address cap, optional price override).
# - After allowlist_end: standard open edition minting.
# - Full FA2 + marketplace: listings, offers, buy, blacklist, withdraw.
# - Per-token royalty and min offer stored in token_config.
...
def main():
    ...
    class BowersAllowlistFA2(sp.Contract):
...
@sp.add_test()
def test():
    scenario = sp.test_scenario("BowersAllowlist", main)
    admin = sp.test_account("admin")
    alice = sp.test_account("alice")
    bob = sp.test_account("bob")

    c = main.BowersAllowlistFA2(
        admin=admin.address,
