...
def main():
    ...
    OfferType: type = sp.record(
        ...
        expiry=sp.timestamp, created_at=sp.timestamp, active=sp.bool)

    ListingType: type = sp.record(price=sp.mutez, max_qty=sp.nat, min_bps=sp.nat)

    TokenConfigType: type = sp.record(
        creator=sp.address, mint_price=sp.mutez,
        mint_end=sp.option[sp.timestamp], mint_paused=sp.bool)
    ...
    class BowersOpenEditionFA2(sp.Contract):
        ...
        # ---- Token creation + open-edition minting ----

        @sp.entrypoint
        def create_token(self, params):
            sp.cast(params, sp.record(token_info=sp.map[sp.string, sp.bytes], creator=sp.address, mint_price=sp.mutez, mint_end=sp.option[sp.timestamp]))
            ...
            self.data.token_config[tid] = sp.record(creator=params.creator, mint_price=params.mint_price, mint_end=params.mint_end, mint_paused=False)
        ...
        def set_mint_paused(self, params):
            ...
            assert cfg.mint_end.is_none(), "NOT_FOREVER_DURATION"
            cfg.mint_paused = params.paused
            self.data.token_config[params.token_id] = cfg

        @sp.entrypoint
        def mint_editions(self, params):
            sp.cast(params, sp.record(token_id=sp.nat, qty=sp.nat, to_=sp.option[sp.address]))
            assert params.token_id in self.data.token_config, "TOKEN_UNDEFINED"
            cfg = self.data.token_config[params.token_id]
            assert params.qty > 0, "BAD_QTY"
            assert cfg.mint_price > sp.mutez(0), "MINT_PRICE_ZERO"
            if cfg.mint_end.is_some():
                assert sp.now <= cfg.mint_end.unwrap_some(), "MINT_CLOSED"
            else:
                assert not cfg.mint_paused, "MINT_CLOSED"
            tp = sp.split_tokens(cfg.mint_price, params.qty, 1)
            assert sp.amount == tp, "WRONG_PRICE"
            to_addr = sp.sender
            if params.to_.is_some():
                to_addr = params.to_.unwrap_some()
            self.data.claimable[cfg.creator] = self.data.claimable.get(cfg.creator, default=sp.mutez(0)) + tp
            lk = sp.record(owner=to_addr, token_id=params.token_id)
            cb = self.data.ledger.get(lk, default=sp.nat(0))
            self.data.ledger[lk] = cb + params.qty
        ...
        # ---- Offers (partial fill, fixed 7-day lifetime) ----

        @sp.entrypoint
        def make_offer(self, params):
            sp.cast(params, sp.record(token_id=sp.nat, qty=sp.nat))
            assert params.token_id in self.data.token_config, "TOKEN_UNDEFINED"
            assert params.qty > 0, "BAD_QTY"
            ...
            mp = self.data.token_config[params.token_id].mint_price
            if mp > sp.mutez(0):
                assert up >= mp, "OFFER_BELOW_MINT_PRICE"
            ...
            self.data.offers[oid] = sp.record(
                ...
                expiry=sp.add_seconds(ca, 604800), created_at=ca, active=True)
        ...
        def close_offer(self, offer_id):
            ...
            if o.buyer == sp.sender:
                ...
                assert sp.sender == self.data.admin, "NOT_BUYER_OR_ADMIN"
                assert sp.now > o.expiry, "NOT_EXPIRED"
            o.active = False
...
@sp.add_test()
def test():
    scenario = sp.test_scenario("BowersOpenEditionFA2", main)
